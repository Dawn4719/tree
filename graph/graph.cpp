...
void Graph::loadGraphFromVector(std::vector<std::pair<int, int>> &E) {

    // vertices_count_ >> edges_count_;
    int max_size = 0;
    std::unordered_map<int, unsigned int> mp;
    vertices_count_ = 0;
    for (auto& [u, v] : E) {
        if (mp.find(u) == mp.end()) mp[u] = vertices_count_++;
        if (mp.find(v) == mp.end()) mp[v] = vertices_count_++;
    }
    ...
    LabelID max_label_id = 0;
    std::vector<int> degree(vertices_count_);
    std::vector<ui> neighbors_offset(vertices_count_, 0);
    for (auto& [u, v] : E) {
        labels_[u] = 0;
        labels_[v] = 0;
    ...
    for (int u = 0; u < vertices_count_; ++u) {
        offsets_[u + 1] = offsets_[u] + degree[u];

        if (degree[u] > max_degree_) {
            max_degree_ = degree[u];
        }

        if (labels_frequency_.find(0) == labels_frequency_.end()) {
            labels_frequency_[0] = 0;
            if (0 > max_label_id)
                max_label_id = 0;
        }

        labels_frequency_[0] += 1;
    }

    for (auto& [begin, end] : E) {
        // Read edge.
        ui offset = offsets_[begin] + neighbors_offset[begin];
        neighbors_[offset] = end;
